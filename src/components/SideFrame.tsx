...
const IconDiv = ({iconName}: { iconName: string }) => (
    <div
        ...
        style={{ backgroundImage: `url(${process.env.PUBLIC_URL}/assets/${iconName}.png)` }}
    />
);
...
const MenuItem = ({name}: {name: string}) => (
    <div className={`menu-item-container ${page===name ? "menu-selected" : ""} center`}
  ...
  return (
    ...
    <div className="center column info">
        ...
        <div>by <span>@SimpleLogix</span></div>
    </div>
</div>
  );
};
